...
function extractTags(record) {
  ...
  if (record.journal && record.issnL && record.venueType === "journal") {
    const issns = record.issns?.length ? record.issns : [record.issnL];
    for (const issn of issns) {
      tags.push({ category: "journal", value: record.journal, ext_id: issn });
    }
  } else if (record.journal) {
    tags.push({ category: "non-journal", value: record.journal, ext_id: record.issnL || null });
    if (record.venueType === "repository") {
      tags.push({ category: "repository", value: record.journal, ext_id: record.issnL || null });
    }
  }
  if (record.type) {
    tags.push({ category: "type", value: canonicalize("type", record.type) });
  }
  // publisher, venue, year, source are properties on doi_records — filters, not tags
  return tags;
}
