 ...
 *  tenantId; an event re-pushes every live stream for that tenant. */
...
export function attachStreamWs(server: HttpServer): void {
  ...
  // Governor write → invalidate that tenant's live charts. Fire-and-forget;
  // the registry recomputes and re-pushes a full directive.value per stream.
