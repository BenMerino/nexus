...
const tagMetaQuery = db.prepare(`
  SELECT
    ...
    CAST(SUM(CASE WHEN d.open_access = 1 THEN 1 ELSE 0 END) AS REAL) / COUNT(*) AS oa_pct,
    GROUP_CONCAT(d.abstract, ' ||| ') AS abstracts
  FROM tags t
  JOIN doi_records d ON d.id = t.doi_record_id
  GROUP BY t.category, t.value
`);

function getGraphMetadata(req, res) {
  const rows = tagMetaQuery.all();
  const tagMeta = {};
  for (const r of rows) {
