...
## EventBus
- Typed via `GovernorEventMap` in `apps/api/src/services/EventBus.ts`. Channel naming: `<domain>.<action>` — lowercase domain, camelCase action (`publication.upserted`, `ingestion.completed`).
- **One owner per channel.** Exactly one file emits any given channel (`PublicationGovernor` emits `publication.*`).
...
## Naming & size
- PascalCase = class/manifest files (`PublicationGovernor.ts`, `PublicationActions.ts`). kebab/camel = `{Domain}Logic.ts` helpers.
