...
**One-family model (Apple/Google):** one sans (**Inter**) for ALL UI text + one mono (**JetBrains Mono**) for technical microcopy. No serif in the product UI. `--display` is a named slot that currently equals `--sans` (a future editorial serif is a one-line swap in `shared.css`). The webfonts are loaded once for every page via `FONT_LINKS` in `vite.config.ts` (`transformIndexHtml` boot inject, `display=swap`) — without it the families fall back to system fonts.
...
- **Never hand-write raw type** (`font-size:18px`, `fontWeight:700`, `font-family:Inter`) — including inline `<style>` and `style=` attrs in `.html`. Use `var(--text-*/--weight-*/--font-*)` or `<BaseText variant=…>`. Enforced: **N3-type** hard-blocks raw type in `apps/web/**` (`.ts/.tsx/.js/.css/.html`); **N3-gen** hard-blocks a drifted artifact. Exempt: `ui/primitives/`, the source/generator, `shared.css`/`dna-defaults.css` family stacks, and `ui/graph-engine/` (vendored + SVG-numeric — Zincro-owned debt).
