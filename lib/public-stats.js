...
async function getYearByIndexation(tenantId) {
  const r = await sql`
    ...
    FROM doi_records d
    LEFT JOIN tags t ON t.doi_record_id = d.id AND t.category = 'indexed_in'
    WHERE d.tenant_id = ${tenantId} AND d.published IS NOT NULL AND d.published <> ''
    GROUP BY d.id, SUBSTRING(d.published FROM 1 FOR 4)`;
  const PRIORITY = ["WoS", "Scopus", "SciELO"];
  const counts = new Map();
  for (const row of r.rows) {
    if (!row.year) continue;
    const bucket = PRIORITY.find(p => (row.indices || []).includes(p)) || "Other";
    const key = `${row.year}|${bucket}`;
...
async function getPublicStats(scope) {
  const [summary, yearSource, collabs, countries, types, journals, yearRange, bySource, typeByYear, yearByIndex] = await Promise.all([
    getSummary(scope),
  ...
  return { summary, yearSource, collabs, countries, types, journals, yearRange, bySource, typeByYear, yearByIndex };
}

module.exports = {
  getPublicationTypes, getTopJournals, getYearRange, getBySource,
