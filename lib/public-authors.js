...
async function getAuthorsDirectory(tenantId, tenantRor) {
  ...
  const records = await sql`
    SELECT d.id, d.citation_count, d.affiliations
    FROM doi_records d
  ...
  const citesById = new Map(records.rows.map(r => [r.id, parseInt(r.citation_count) || 0]));
  ...
  for (const t of tags.rows) {
    ...
    if (!byAuthor.has(key)) byAuthor.set(key, { name: t.name, orcid: t.orcid || null, citations: [] });
    byAuthor.get(key).citations.push(citesById.get(t.doi_record_id) || 0);
  }
