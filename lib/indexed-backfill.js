...
const OPENALEX_INDEX_MAP = {
  scopus: "Scopus", wos: "WoS", doaj: "DOAJ", scielo: "SciELO", pubmed: "PubMed",
};
...
async function tagIndexedInFromOpenAlex(recordId, indexedIn, issnL) {
  if (!recordId || !indexedIn?.length) return [];
  const tagged = [];
  for (const raw of indexedIn) {
    const src = OPENALEX_INDEX_MAP[String(raw).toLowerCase()];
    if (!src) continue;
    await sql`INSERT INTO tags (doi_record_id, category, value, ext_id)
      VALUES (${recordId}, 'indexed_in', ${src}, ${issnL || null})
...
module.exports = { backfillIndexationTags, clearIndexationTagsForSource, tagIndexationForRecord, tagIndexedInFromOpenAlex };
