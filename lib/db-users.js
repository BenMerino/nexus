...
async function listUsers(tenantId) {
  if (tenantId) {
    const r = await sql`SELECT id, username, full_name, email, role, tenant_id, position, faculty, titles, orcid, active FROM users WHERE tenant_id = ${tenantId} ORDER BY full_name`;
    return r.rows;
  }
  const r = await sql`SELECT u.id, u.username, u.full_name, u.email, u.role, u.tenant_id, u.position, u.orcid, u.active, t.name as tenant_name FROM users u LEFT JOIN tenants t ON u.tenant_id = t.id ORDER BY u.id`;
  return r.rows;
}
...
async function createTenant(name, rorId, parentId) {
  const pid = parentId || null;
  const r = await sql`
    INSERT INTO tenants (name, ror_id, parent_id) VALUES (${name}, ${rorId}, ${pid}) RETURNING id`;
  return r.rows[0].id;
}
