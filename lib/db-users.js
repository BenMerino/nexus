...
async function listUsers(tenantId) {
  if (tenantId) {
    const r = await sql`SELECT id, username, full_name, email, role, tenant_id, position, faculty, titles, orcid, active, grado_academico, horas_permanencia, department, profile_category FROM users WHERE tenant_id = ${tenantId} ORDER BY full_name`;
    return r.rows;
  }
  const r = await sql`SELECT u.id, u.username, u.full_name, u.email, u.role, u.tenant_id, u.position, u.orcid, u.active, u.grado_academico, u.horas_permanencia, u.department, u.profile_category, t.name as tenant_name FROM users u LEFT JOIN tenants t ON u.tenant_id = t.id ORDER BY u.id`;
  return r.rows;
}
