 ...
 *  journal tags by their display name and pick one canonical ISSN (smallest
 *  string) so all siblings collapse to a single node. */
function canonicalJournalIssns(tags, synonymMap) {
  const canonical = new Map(); // name(lc) → canonical issn
  const byName = new Map();    // name(lc) → Set of issns
  for (const t of tags) {
    if (t.category !== "journal" || !t.ext_id) continue;
    const name = (synonymMap.get(`journal:${t.value}`) || t.value || "").toLowerCase();
    if (!name) continue;
...
async function buildGraph(scope) {
  ...
  for (const t of tags) {
    ...
    if (t.category === "journal" && resolved) {
      const canon = journalCanonIssn.get(resolved.toLowerCase());
