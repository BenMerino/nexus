...
const { extractSourceIndices } = require("./normalize-openalex");
...
async function fetchAndStore(doi, submissionId) {
  ...
  for (const src of extractSourceIndices(sources.openalex || {})) {
    await insertTag(dbRecord.id, "source", src, null);
  }
