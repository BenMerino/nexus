...
const { tagIndexationForRecord, tagIndexedInFromOpenAlex } = require("./indexed-backfill");
...
async function fetchAndStore(doi, submissionId) {
  ...
  await tagIndexedInFromOpenAlex(dbRecord.id, record.indexedIn, record.issnL);
