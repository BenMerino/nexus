...
function findAllCols(header, needle) {
  const out = [];
  header.forEach((h, i) => { if (h.includes(needle)) out.push(i); });
  return out;
}

function extractEntries(csvText) {
  const { header, rows } = parseCsv(csvText);
  const issnCols = findAllCols(header, "issn");
  const titleIdx = findCol(header, ["title", "journal", "name"]);
  if (!issnCols.length) return [];
  const seen = new Map();
  for (const row of rows) {
    const name = titleIdx >= 0 ? row[titleIdx] : null;
    for (const col of issnCols) {
      const rawIssns = (row[col] || "").split(/[\s,;]+/).filter(Boolean);
...
async function replaceIndex(source, entries) {
  const { canonicalSource } = require("./indexation-sources");
  ...
  for (let i = 0; i < entries.length; i += CHUNK) {
    ...
    const issns = slice.map(e => e.issn_l);
    const names = slice.map(e => e.journal_name);
    await sql`
      INSERT INTO indexed_journals (issn_l, source, journal_name)
      SELECT unnest(${issns}::text[]), ${canon}, unnest(${names}::text[])
...
async function listCounts() {
  const { SOURCES } = require("./indexation-sources");
  const r = await sql`
...
module.exports = {
  normalizeIssn, parseCsv, extractEntries,
