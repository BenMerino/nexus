...
Adapted from Zincro's doctrine, sized to Nexus. We took the *structure* (a small set of named invariants + on-demand subsystem rules + a heuristics manifest) and the *spirit* (determinism, isolation, token-sourced visuals) — not the letter (no 15-Governor DGA, no EventBus; Nexus is Express handlers + a `lib/` data layer).
