...
- **REMAINING P4**: VenueGovernor.setIndexation (the indexed_in/venue-type tag WRITES — `indexed-backfill.tagIndexationForRecord`, `venue-type-backfill`, `handlers/indexation`) → write venue flags directly instead of tags. (VenueGovernor.merge already effectively exists as mergeInstitutionSynonym; wrap as a governor in a later DGA-formalize pass — not a drop blocker.)
- **NEXT: P5** — stop the core tag write path (`db.js insertTag`/`deleteTagsForRecord`/`resolveByExtId` called from store.js/store-openalex.js) once indexed_in writes are repointed; then migration `008_drop_tags.sql` (tags + tag_synonyms + tag_dismissed_pairs + doi_records view). Remove remaining dead code (institution-detail.js, node-detail-helpers.js, legacy graph-builder.js baseline + getAllTags).
