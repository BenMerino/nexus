...
- **Still TODO:** §2 (wrap remaining reads as Statistician/Claustro resolvers), §3 (frontend consumes server chart specs). VenueGovernor `.upsert`/`.merge` (ISSN-dedup) and AuthorGovernor `.upsert`/`.merge` not yet built (no live caller needs them — the per-author/venue resolution ingestion does happens *inside* `syncRecordEntities`, which PublicationGovernor already owns).
