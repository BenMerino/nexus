    ...
    <div className="detail-panel" style={{
      ...
      <div className="detail-head" style={{ paddingBottom: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--fg-dim)', marginTop: 6, display: 'flex', gap: 10, flexWrap: 'wrap', fontFamily: 'var(--mono)' }}>
      ...
      {Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([group, items]) => (
        <div key={group} style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 500, color: COLORS[group], marginBottom: 4, fontFamily: 'var(--mono)' }}>
            {group} ({items.length})
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {items.slice(0, 25).map(({ node: item, weight }) => (
              <span key={item.id} onClick={() => onSelectNode(item.id)}
                ...
                {item.label}
                {weight > 1 && <span style={{ fontSize: 9, opacity: 0.75, fontWeight: 500 }}>{weight}</span>}
              </span>
            ))}
            {items.length > 25 && <span style={{ fontSize: 11, color: 'var(--fg-dim)' }}>+{items.length - 25} more</span>}
          </div>
        </div>
      ))}
      ...
      {allDois.length > 0 && (
        <div style={{ marginTop: 14, borderTop: '1px solid var(--border-soft)', paddingTop: 10 }}>
          ...
          {allDois.slice(0, 15).map((d, i) => (
            <div key={i} style={{ fontSize: 11, color: 'var(--fg-muted)', padding: '3px 0', borderBottom: '1px solid var(--border-soft)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'var(--mono)' }}>{d}</div>
          ))}
          {allDois.length > 15 && <div style={{ fontSize: 11, color: 'var(--fg-dim)', marginTop: 6 }}>+{allDois.length - 15} more</div>}
        </div>
      )}
    </div>
  );
